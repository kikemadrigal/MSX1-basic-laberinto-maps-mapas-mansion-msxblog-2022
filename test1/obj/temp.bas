...
1'Rutina posicionar player y enemigos según el mapa
    ...
    1' Debug
    8320 if mw=0 and ms=1 then x=13*8:y=9*8:tx=10:ty=15:gosub 6100:ex=10*8:ey=15*8:gosub 6200:bx=7*8:by=4*8:bt=1
    1 '8320 if mw=0 and ms=1 then x=1*8:y=18*8:tx=10:ty=15:gosub 6100:ex=10*8:ey=15*8:gosub 6200:bx=7*8:by=4*8:bt=1
    1 'Debug'
    8330 if mw=0 and ms=2 then x=13*8:y=10*8:tx=10:ty=7:gosub 6100:ex=10*8:ey=1*8:gosub 6200:bx=10*8:by=10*8:bt=0
    1 '8330 if mw=0 and ms=2 then x=1*8:y=14*8:tx=10:ty=7:gosub 6100:ex=10*8:ey=1*8:gosub 6200:bx=10*8:by=10*8:bt=0
    
    
    1 'LEVEL 3 O 1-0' 
    8340 if mw=1 and ms=0 then x=1*8:y=18*8:tx=10:ty=4:gosub 6100:ex=10*8:ey=4*8:gosub 6200:bx=17*8:by=13*8:bt=1
    1 '8340 if mw=1 and ms=0 then x=14*8:y=7*8:tx=10:ty=4:gosub 6100:ex=10*8:ey=4*8:gosub 6200:bx=17*8:by=13*8:bt=1
    1 'LEVEL 4 O 1-1'
    1 'debug'
